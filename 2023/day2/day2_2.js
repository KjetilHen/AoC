...
function parseGames(input) {
    ...
    for(const stringInput of input) {
        ...
        const groupedDrawsByColour = groupByColour(draws);
        const power = getPowerOfSet(groupedDrawsByColour);
        total += power;
    }
    console.log(total);
}
    ...
    for (const draw of draws) {
        for (const key in draw) {
            if (Object.hasOwnProperty.call(draw, key)) {
                const element = draw[key];
                if (key === 'red') { redGroup.push(+draw[key])}
                if (key === 'green') { greenGroup.push(+draw[key])}
                if (key === 'blue') { blueGroup.push(+draw[key])}
            }
        }
    }
    // returns Object of arrays each containing the number of cubes from one colour
    return {red: redGroup, green: greenGroup, blue: blueGroup};
}

function getPowerOfSet(draws) {
    console.log(draws['green']);
    // get max value of each color
    const redMaxValue = Math.max(...draws['red']);
    const greenMaxValue = Math.max(...draws['green']);
    const blueMaxValue = Math.max(...draws['blue']);
    // return the multiplication of these three factors;
    return redMaxValue * greenMaxValue * blueMaxValue;
}
